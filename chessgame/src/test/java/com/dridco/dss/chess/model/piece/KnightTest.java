...
public class KnightTest extends ChessPieceTest {
	...
	public void testCanMoveTo_HavingInvalid_DestLocation_shouldReturn_false() {
		Assert.assertFalse(whiteKnightPiece.canMoveTo(Coordinates.B1, Coordinates.B4));
		Assert.assertFalse(blackKnightPiece.canMoveTo(Coordinates.B8, Coordinates.B4));
	}
	
	@Test
	public void testCanMoveTo_HavingValid_DestLocation_shouldReturn_true() {
		Assert.assertTrue(whiteKnightPiece.canMoveTo(Coordinates.B1, Coordinates.A3));
		Assert.assertTrue(whiteKnightPiece.canMoveTo(Coordinates.B1, Coordinates.C3));
		Assert.assertTrue(blackKnightPiece.canMoveTo(Coordinates.B8, Coordinates.A6));
		Assert.assertTrue(blackKnightPiece.canMoveTo(Coordinates.B8, Coordinates.C6));
	}
